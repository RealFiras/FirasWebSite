...
import { LockKeyhole, Trophy, ShieldCheck, Radio, Search, ScanSearch } from "lucide-react";
...
const FALLBACK_ICONS: Record<string, LucideIcon> = {
    ...
    "CTF Player": Trophy,
    "Blue Team": ShieldCheck,
    SOC: Radio,
    "Defensive Security": ShieldCheck,
    "Incident Response": Radio,
    OSINT: Search,
    "Threat Analysis": ScanSearch,
    Reconnaissance: Search,
    "Digital Investigation": ScanSearch,
};

function StackIcon({ item, size, className }: { item: StackItem; size: number; className?: string }) {
    if (item.icon) {
        return <Image src={item.icon} alt={item.name} width={size} height={size} unoptimized={item.icon.endsWith(".svg")} className={className} />;
    }

    const Icon = FALLBACK_ICONS[item.name] ?? ShieldCheck;
                                        ...
                                        <HoverCard key={item.name} openDelay={50} closeDelay={50}>
                                            ...
                                            >
                                                ...
                                                <div className="relative p-3 rounded-xl bg-secondary/50 ring-1 ring-border/50 shadow-inner group-hover:scale-110 transition-transform duration-500">
                                                    <StackIcon item={item} size={36} className="drop-shadow-lg" />
                                                </div>
